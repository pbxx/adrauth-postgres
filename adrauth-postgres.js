  ...
  PGLink: class {
    ...
    insert(table, object) {
      return new Promise((resolve, reject) => {
        var console = kind.logger("dbactions.insert");
        try {
          var fName = "db.insert";
          ...
          if (typeof object == "object" && !Array.isArray(object)) {
            if (typeof table == "string") {
              ...
              reject(
                `[ERR: ${fName}] First argument must be of type 'string', got '${typeof table}'.`
              );
            }
          } else {
            reject(
              `[ERR: ${fName}] Second argument must be of type 'object', got '${typeof object}'.`
            );
          }
        } catch (err) {
          reject(err);
    ...
    delete(table, cases, opArray) {
      return new Promise((resolve, reject) => {
        var console = kind.logger("dbactions.delete");
        try {
          var fName = "db.delete";
          //object keys will become columns, object values will be written to those columns
          //make sure <object> is an actual object
          if (typeof cases == "object" && !Array.isArray(cases)) {
            if (typeof table == "string") {
                    ...
                    if (opArray) {
                      if (Array.isArray(opArray)) {
                        ...
                        if (typeof opArray[oaIncrement] == "string") {
                          ...
                          reject(
                            `[ERR: ${fName}] All values in third argument array must be string.`
                          );
                        }
                      } else {
                        //opArray must be array
                        reject(
                          `[ERR: ${fName}] If third argument is used, it must be an array. Got '${typeof opArray}'.`
                        );
                      }
                    } else {
                      //no opArray was passed at all, or it was falsy
                      valCases +=
                        `${ncase} = '` + "$" + valDollarIncrement + "'";
                      valArray.push(cases[ncase]);
                      valDollarIncrement++;
                    }
                  } else {
              ...
              this.pool.connect((err, client, release) => {
                ...
                console.log(query);
                if (globals.consoleLog) {
                  console.log("[INFO db.delete]", valArray, valCases, query);
                }
            ...
            reject(
              `[ERR: ${fName}] Second argument must be of type 'object', got '${typeof cases}'.`
            );
          }
        } catch (err) {
          reject(err);
    ...
    selectAll(table, cases, opArray) {
      return new Promise((resolve, reject) => {
        var console = kind.logger("dbactions.selectAll");
        try {
          ...
          if (typeof table == "string") {
            ...
            if (typeof cases == "object" && !Array.isArray(cases)) {
              this.pool.connect((err, client, release) => {
                ...
                if (globals.consoleLog) {
                  console.log(`[INFO ${fName}]`, valArray, valCases, query);
                }
                client.query(query, valArray, (err, res) => {
                  if (err) {
                    //err writing to db
              ...
              this.pool.connect((err, client, release) => {
                ...
                if (globals.consoleLog) {
                  console.log(`[INFO ${fName}]`, valCases, query);
                }
    ...
    rowCountEstimate(table) {
      return new Promise((resolve, reject) => {
        var console = kind.logger("dbactions.rowCountEstimate");
        try {
    ...
    selectCols(table, cols, cases, opArray) {
      return new Promise((resolve, reject) => {
        var console = kind.logger("dbactions.selectCols");
        try {
          ...
          if (typeof table == "string") {
            if (typeof cols == "string") {
                    ...
                    if (ncase == casesArr[casesArr.length - 1]) {
                      ...
                      if (opArray) {
                        if (Array.isArray(opArray)) {
                          ...
                          //opArray must be array
                          reject(
                            `[ERR: ${fName}] If fourth argument is used, it must be an array. Got '${typeof opArray}'.`
                          );
                        }
                      } else {
                        //no opArray was passed at all, or it was falsy
              ...
              reject(
                `[ERR: ${fName}] Second argument must be of type 'string', got '${typeof cols}'.`
              );
            }
          } else {
            reject(
    ...
    update(table, values, cases, opArray) {
      return new Promise((resolve, reject) => {
        var console = kind.logger("dbactions.update");
        try {
          var fName = "db.update";
          //object keys will become columns, object values will be written to those columns
          //make sure <object> is an actual object
            ...
            if (typeof values == "object" && !Array.isArray(values)) {
              ...
              if (typeof cases == "object" && !Array.isArray(cases)) {
                ...
                this.pool.connect((err, client, release) => {
                  ...
                  if (globals.consoleLog) {
                    console.log(`[INFO ${fName}]`, valArray, valValues);
                    console.log(`[INFO ${fName}]`, query);
                  }
                  client.query(query, valArray, (err, res) => {
                    if (err) {
              ...
              reject(
                `[ERR: ${fName}] Second argument must be of type 'object', got '${typeof values}'.`
              );
            }
          } else {
            reject(
    ...
    increment(table, values, inc, cases, opArray) {
      return new Promise((resolve, reject) => {
        var console = kind.logger("dbactions.increment");
        try {
              ...
              if (inc && typeof inc == "number") {
                ...
                if (typeof cases == "object" && !Array.isArray(cases)) {
                  this.pool.connect((err, client, release) => {
                    ...
                    if (globals.consoleLog) {
                      console.log(`[INFO ${fName}]`, valArray, valCases);
                      console.log(`[INFO ${fName}]`, query);
                    }
                    client.query(query, valArray, (err, res) => {
                      if (err) {
                  ...
                  this.pool.connect((err, client, release) => {
                    ...
                    if (globals.consoleLog) {
                      console.log(`[INFO ${fName}]`, valArray);
                      console.log(`[INFO ${fName}]`, query);
                    }
                    client.query(query, valArray, (err, res) => {
                      if (err) {
                ...
                reject(
                  `[ERR: ${fName}] Third argument must be of type 'number', got '${typeof inc}'.`
                );
              }
            } else {
              reject(
                `[ERR: ${fName}] Second argument must be of type 'string', got '${typeof values}'.`
              );
            }
          } else {
            reject(
    ...
    logRequest(ip, ipInfo, path) {
      return new Promise((resolve, reject) => {
        var console = kind.logger("dbactions.logRequest");
        //console.log(ip, ipInfo, path)
        //check if ip exists in DB at path
        module.exports
          .selectCols("uniquevisitors", "uvid", { ip })
          .then((resp) => {
            if (resp.rows.length > 0) {
              ...
              if (resp.rows.length > 0) {
                ...
                module.exports
                  .insert("requestlogs", { ip, path })
                  .then((resp) => {
                    module.exports
                      .increment("uniquevisitors", "reqs", 1, { ip })
                      .then((resp) => {
                        resolve(resp);
                      })
                      .catch((err) => {
